...
const Menu = () => {
  ...
  const categoryOrder = ["Starters → Soups", "Chinese Veg Starter", "Chinese Non-Veg Starter", "Momos", "Sharings", "Main Course Chinese → Rice", "Main Course Chinese → Noodles", "Pasta", "Pizza", "Burger", "Sandwiches", "Healthy & Light → Salads", "Healthy & Light → Juices", "Desserts", "Beverages → Mojitos", "Beverages → Milkshakes", "Beverages"];
  ...
  const menuItems: MenuItem[] = menuData.map(item => ({
    ...
    // Keep full category name with subcategory
  ...
  const filteredItems = useMemo(() => {
    return menuItems.filter(item => {
      // Search filter
      const matchesSearch = searchQuery === "" || item.name.toLowerCase().includes(searchQuery.toLowerCase()) || item.description.toLowerCase().includes(searchQuery.toLowerCase()) || item.category.toLowerCase().includes(searchQuery.toLowerCase()) || item.subCategory.toLowerCase().includes(searchQuery.toLowerCase());

      // Category filter
      const matchesCategory = filters.categories.length === 0 || filters.categories.includes(item.category);

      // Dietary filter
      const matchesDietary = filters.dietary.length === 0 || filters.dietary.includes("veg") && item.veg || filters.dietary.includes("non-veg") && !item.veg;

      // Spice level filter
      const matchesSpice = filters.spiceLevel === "all" || item.spiceLevel === filters.spiceLevel;
    ...
    return <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-24">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[...Array(12)].map((_, i) => <div key={i} className="aspect-[4/5] bg-muted rounded-2xl animate-pulse" />)}
          </div>
        </main>
      </div>;
  }
  return <>
      <Helmet>
        ...
        <meta name="description" content="Explore Sunroof Café's complete menu with 145+ dishes including Indo-Chinese starters, pasta, pizza, burgers, desserts, and beverages. Order online or visit us today!" />
        <meta property="og:title" content="Sunroof Café — Full Menu" />
        <meta property="og:description" content="Browse our extensive menu featuring soups, starters, momos, rice, noodles, pasta, pizza, burgers, salads, desserts, and drinks." />
      </Helmet>

      <MenuSchema items={menuItems} />

      {/* Floating Food Animations */}
      <MenuAnimationCanvas isEnabled={animationsEnabled} activeFilter={filters.categories[0]} />
        ...
        <main className="container mx-auto px-4 py-24 relative z-10" id="main-content">
          ...
          <div className="mx-auto max-w-4xl mb-12 space-y-4">
            ...
            <div className="flex items-center justify-center gap-3 flex-wrap">
              <MenuFilters filters={filters} onFiltersChange={setFilters} categoryCounts={categoryCounts} />
          ...
          {Object.keys(groupedItems).length > 0 ? <div className="space-y-16">
              {Object.entries(groupedItems).map(([category, items]) => <section key={category} id={category.toLowerCase().replace(/\s+/g, '-')}>
                  <div className="mb-8">
            ...
            </div> : <MenuGrid items={[]} onItemClick={setSelectedItem} />}
        </main>

        <Footer />

        {/* Dish Detail Modal */}
        <DishModal item={selectedItem} isOpen={selectedItem !== null} onClose={() => setSelectedItem(null)} />
      </div>
    </>;
};
export default Menu;